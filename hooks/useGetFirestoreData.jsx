...
import {doc, getDoc, getDocs, collection, where, query, limit, orderBy} from 'firebase/firestore';
import {db} from '../firebase';

 const useGetFirestoreData = (c, documentId, whereStatement, orderBy_, orderType, limit_) => {
    ...
    try {
      ...
      if (documentId) {
        ...
        if (whereStatementRef.current) {
          queryWhere = 
          whereStatementRef.current?.lhs && whereStatementRef.current?.op && whereStatementRef.current?.rhs &&
          where(whereStatementRef.current.lhs , whereStatementRef.current.op, whereStatementRef.current.rhs);
        }
        ...
        const filteredQueryConstraints = Object.entries(queryConstraints).reduce((acc, [key, value]) => {
          if (value !== null && value !== undefined) {
            acc[key] = value;
          }
          return acc;
        }, {});
        const filteredQueryConstraintsArray = Object.values(filteredQueryConstraints);
