...
const usePagination = (data, elementsPerPage, changeURL, maxSiblings, hideArrows, containerClass, listClass, pageClass) => {
  // Adding the classes if they were provided
  const containerClasses = containerClass ? 'pagination ' + containerClass : 'pagination'
  const listClasses = listClass ? 'pagination-list ' + listClass : 'pagination-list'
  const pageClasses = pageClass ? 'pagination-link ' + pageClass : 'pagination-link'
  ...
  */
  const [currentPage, setCurrentPage] = useState(changeURL ? parseInt(location.pathname.split('/').pop()) : 1)
  const [pages, setPages] = useState(Array.from({length: pagesNumber}, (_, index) => index + 1 )); // The pages state array
  ...
  */
  ...
  const modifiedData = data.slice((currentPage - 1) * elementsPerPage, (currentPage - 1) * elementsPerPage + elementsPerPage);
